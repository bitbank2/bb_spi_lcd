...
// RGB panel 480x480 - Unexpected Maker Square Display
...
const BB_RGB rgbpanel_WAVESHARE_480x480 = {
	42 /* CS */, 2 /* SCK */, 1 /* SDA */,
...
// swap DMA buffers
	...
	if (pLCD->bUseDMA && (iMode == MODE_COMMAND || !(iFlags & DRAW_WITH_DMA)))
	{
		...
		while (iLen)
		{
			int l = iLen;
			if (l > 4000)
			{ // transmit maximum length (full duplex mode)
				l = 4000;
			}
...
void spilcdWritePixelsMasked(SPILCD *pLCD, int x, int y, uint8_t *pData, uint8_t *pMask, int iCount, int iFlags)
{
	...
	while (i < iCount && s < pEnd)
	{
		...
		pix_count = 0;
		while (bit_count && (c & 0x80) == 0)
		{ // count 0's
			if (c == 0)
			{ // quick count remaining 0 bits
				pix_count += bit_count;
				bit_count = 0;
				if (s < pEnd)
				{
...
int spilcdInit(SPILCD *pLCD, int iType, int iFlags, int32_t iSPIFreq, int iCS, int iDC, int iReset, int iLED, int iMISOPin, int iMOSIPin, int iCLKPin, int bUseDMA)
{
	...
	if (pLCD->pFont != NULL && iSPIFreq != 0)
	{ // the structure is probably not initialized
		memset(pLCD, 0, sizeof(SPILCD));
	}
...
start_of_init:
	...
	case LCD_ILI9488:
	{
		...
		if (pLCD->iLCDType == LCD_ILI9488)
		{ // slightly different init sequence
...
void spilcdSetPosition(SPILCD *pLCD, int x, int y, int w, int h, int iFlags)
{
...
#endif
	pLCD->iWindowX = pLCD->iCurrentX = x;
	pLCD->iWindowY = pLCD->iCurrentY = y;
	pLCD->iWindowCX = w;
	pLCD->iWindowCY = h;
	...
	if (y != pLCD->iOldY || h != pLCD->iOldCY)
	{
		...
		if (pLCD->iCMDType == CMD_TYPE_SITRONIX_8BIT)
		{
			...
			y = y + h;
			if ((y - pLCD->iMemoryY) > pLCD->iCurrentHeight - 1)
				y = pLCD->iMemoryY + pLCD->iCurrentHeight;
...
void qspiSendDATA(SPILCD *pLCD, uint8_t *pData, int iLen, int iFlags)
{
	...
	trans[0].addr = 0;
	if (bSetPosition)
	{ // first data after a setPosition
		trans[0].addr = 0x2c00;
		bSetPosition = 0;
	}
	else
...
int spilcdWriteStringCustom(SPILCD *pLCD, GFXfont *pFont, int x, int y, char *szMsg, uint16_t usFGColor, uint16_t usBGColor, int bBlank, int iFlags)
{
	...
	while (szMsg[i] && x < pLCD->iCurrentWidth)
	{
		...
		if (bBlank)
		{ // erase the areas around the char to not leave old bits
			int miny, maxy;
			c = '0' - font.first;
			...
			{
				d = (uint16_t *)ucRXBuf;
				for (tx = 0; tx < pGlyph->xOffset && tx < cx; tx++)
				{ // left padding
					*d++ = usBGColor;
				}
		...
		{ // just draw the current character box fast
...
int spilcdWriteStringFast(SPILCD *pLCD, int x, int y, char *szMsg, unsigned short usFGColor, unsigned short usBGColor, int iFontSize, int iFlags)
{
	...
	{
		iLen = strlen(szMsg);
		if ((12 * iLen) + x > pLCD->iCurrentWidth)
			iLen = (pLCD->iCurrentWidth - x) / 12; // can't display it all
		if (iLen < 0)
			return -1;
		iStride = iLen * 12;
		spilcdSetPosition(pLCD, x, y, iStride, 16, iFlags);
		for (k = 0; k < 8; k++)
		{ // create a pair of scanlines from each original
			uint8_t ucMask = (1 << k);
			usD = (uint16_t *)pDMA;
			...
			for (i = 0; i < iLen; i++)
			{
				...
				for (j = 1; j < 6; j++)
				{
					...
					if (j < 5)
					{
						...
						if ((c0 & ucMask) && (~c1 & ucMask) && (~c0 & ucMask1) && (c1 & ucMask1))
						{ // first diagonal condition
							usD[iStride + 2] = usFG;
						}
						else if ((~c0 & ucMask) && (c1 & ucMask) && (c0 & ucMask1) && (~c1 & ucMask1))
						{ // second condition
							usD[iStride + 1] = usFG;
						}
						if ((c0 & ucMask2) && (~c1 & ucMask2) && (~c0 & ucMask) && (c1 & ucMask))
						{ // repeat for previous line
							usD[1] = usFG;
						}
	...
	iLen = strlen(szMsg);
	if (iLen <= 0)
		return -1; // can't use this function

	if ((cx * iLen) + x > pLCD->iCurrentWidth)
		iLen = (pLCD->iCurrentWidth - x) / cx; // can't display it all
	if (iLen > 32)
		iLen = 32;
	iStride = iLen * cx * 2;
	for (i = 0; i < iLen; i++)
...
int spilcdWriteString(SPILCD *pLCD, int x, int y, char *szMsg, int usFGColor, int usBGColor, int iFontSize, int iFlags)
{
	...
	if (usFG == usBG)
		usBG = 0; // DEBUG!! no transparent option for now
#ifndef __AVR__
	if (iFontSize == FONT_16x32) // draw 16x32 font
	{
		if (iLen * 16 + x > pLCD->iCurrentWidth)
			iLen = (pLCD->iCurrentWidth - x) / 16;
		if (iLen < 0)
			return -1;
		for (i = 0; i < iLen; i++)
		{
	...
	if (iFontSize == FONT_16x16) // 8x8 stretched to 16x16
	{
		...
		if ((16 * iLen) + x > pLCD->iCurrentWidth)
			iLen = (pLCD->iCurrentWidth - x) / 16; // can't display it all
		if (iLen < 0)
			return -1;
...
void DrawScaledLine(SPILCD *pLCD, int32_t iCX, int32_t iCY, int32_t x, int32_t y, int32_t iXFrac, int32_t iYFrac, uint16_t *pBuf, int iFlags)
{
	...
	if (x < 0)
		x = 0;
	if (x2 >= pLCD->iCurrentWidth)
		x2 = pLCD->iCurrentWidth - 1;
...
void spilcdEllipse(SPILCD *pLCD, int32_t iCenterX, int32_t iCenterY, int32_t iRadiusX, int32_t iRadiusY, uint8_t u8Parts, unsigned short usColor, int bFilled, int iFlags)
{
	...
	if (iCenterX < 0 || iCenterY < 0 || iRadiusX <= 0 || iRadiusY <= 0)
		return;
...
void spilcdDrawLine(SPILCD *pLCD, int x1, int y1, int x2, int y2, unsigned short usColor, int iFlags)
{
	...
	else
	{
		...
		for (y = y1; y1 <= y2; y1++)
		{
			error -= dx;
			if (error < 0)
			{ // x needs to change, write any pixels we traversed
				error += dy;
				iLen = y1 - y + 1;
				// #ifndef ESP32_DMA
				for (i = 0; i < iLen; i++) // prepare color data for max length line
...
int BB_SPI_LCD::rtInit(uint8_t u8MOSI, uint8_t u8MISO, uint8_t u8CLK, uint8_t u8CS)
{
	...
	if (_lcd.iRTMOSI != _lcd.iMOSIPin)
	{ // use bit bang for the touch controller
...
int BB_SPI_LCD::rtReadTouch(TOUCHINFO *ti)
{
	...
	case 180:
		...
		if (y1 < 0)
			y1 = 0;
		else if (y1 >= _lcd.iCurrentHeight)
			y1 = _lcd.iCurrentHeight - 1;
		x1 = (1950 - x) * _lcd.iCurrentWidth;
		x1 /= 1750;
		if (x1 < 0)
			x1 = 0;
		else if (x1 >= _lcd.iCurrentWidth)
			x1 = _lcd.iCurrentWidth - 1;
		break;
	case 90:
	case 270:
...
int BB_SPI_LCD::begin(int iDisplayType, bool bSkipInit)
{
	...
	case DISPLAY_CYD_700: // 7" 800x480 ESP32-S3
		...
		break;
	case DISPLAY_UM_SQUARE: // Unexpected Maker Square Display 4" 480x480
		memset(&_lcd, 0, sizeof(_lcd));
		_lcd.iDCPin = _lcd.iCSPin = -1; // make sure we don't try to toggle these
		_lcd.iLEDPin = -1;
		_lcd.iLCDType = LCD_VIRTUAL_MEM;
		...
		if (!bSkipInit)
		{
			// We could skip the SPI initialisation if we bitbang is user side
			spilcdWritePanelCommands(&rgbpanel_UM_480x480, st7701slist, sizeof(st7701slist));
		}
		spilcdSetBuffer(&_lcd, (uint8_t *)RGBInit((BB_RGB *)&rgbpanel_UM_480x480));
		break;
	case DISPLAY_WAVESHARE: // https://www.waveshare.com/wiki/ESP32-S3-LCD-Driver-Board#Introduction
		memset(&_lcd, 0, sizeof(_lcd));
		_lcd.iDCPin = _lcd.iCSPin = -1; // make sure we don't try to toggle these
		_lcd.iLEDPin = -1;
		_lcd.iLCDType = LCD_VIRTUAL_MEM;
		_lcd.iWidth = _lcd.iCurrentWidth = 480;
		_lcd.iHeight = _lcd.iCurrentHeight = 480;
		spilcdWritePanelCommands(&rgbpanel_WAVESHARE_480x480, st7701slist, sizeof(st7701slist));
		spilcdSetBuffer(&_lcd, (uint8_t *)RGBInit((BB_RGB *)&rgbpanel_WAVESHARE_480x480));
		break;
...
int BB_SPI_LCD::merge(uint16_t *s, uint16_t usTrans, int bSwap565)
{
	...
	if (bSwap565)
	{
		...
		{
			if (d[i] == usTrans)
			{ // replace transparent pixels in current surface
...
int BB_SPI_LCD::captureArea(int dst_x, int dst_y, int src_x, int src_y, int src_w, int src_h, uint16_t *pPixels, int bSwap565)
{
	...
	s = pPixels;
	d = (uint16_t *)_lcd.pBackBuffer;
	dx = dy = 0;
	cx = _lcd.iCurrentWidth;
	cy = _lcd.iCurrentHeight;
...
void BB_SPI_LCD::setRotation(int iAngle)
{
	...
	if (_lcd.iLCDType == LCD_VIRTUAL_MEM)
...
int BB_SPI_LCD::drawSprite(int x, int y, BB_SPI_LCD *pSprite, float fScale, int iTransparentColor, int iFlags)
{
	...
	if (pSprite == NULL || pSprite->_lcd.iLCDType != LCD_VIRTUAL_MEM)
		return BB_ERROR_INV_PARAM; // invalid parameters
	...
	if (x <= -cx || y <= -cy)
		return BB_ERROR_SUCCESS; // nothing to draw
	if (x >= _lcd.iCurrentWidth || y >= _lcd.iCurrentHeight)
		return BB_ERROR_SUCCESS; // nothing to draw
	if (x < 0)
	{
	...
	if (x + cx > _lcd.iCurrentWidth)
	{ // clipped at right edge?
		cx = _lcd.iCurrentWidth - x;
		if (cx < 1)
			return BB_ERROR_INV_PARAM;
	}
	if (y + cy > _lcd.iCurrentHeight)
	{ // clipped at bottom edge?
	...
	if (_lcd.iLCDType == LCD_VIRTUAL_MEM)
		iFlags = DRAW_TO_RAM; // only one possibility (no physical LCD)
	if (iFlags & DRAW_TO_LCD)
	{
	...
	else if (iFlags & DRAW_TO_RAM)
	{ // only RAM, it can have a transparent color
		...
		for (ty = 0; ty < cy; ty++)
		{
			u32XAcc = 0;
			s16 = &s[(u32YAcc >> 16) * pSprite->_lcd.iWidth];
			if (iTransparentColor != -1)
			{ // slower to check each pixel
				for (tx = 0; tx < cx; tx++)
				{
					u16 = s16[(u32XAcc >> 16)];
					if (u16 != u16Trans)
...
int BB_SPI_LCD::drawSprite(int x, int y, BB_SPI_LCD *pSprite, int iTransparentColor, int iFlags)
{
	...
	if (x + pSprite->_lcd.iCurrentWidth > _lcd.iCurrentWidth)
	{ // clipped at right edge?
		cx = _lcd.iCurrentWidth - x;
		if (cx < 1)
			return BB_ERROR_INV_PARAM;
	}
	if (y + pSprite->_lcd.iCurrentHeight > _lcd.iCurrentHeight)
	{ // clipped at bottom edge?
		cy = _lcd.iCurrentHeight - y;
		if (cy < 1)
			return BB_ERROR_INV_PARAM;
...
size_t BB_SPI_LCD::write(uint8_t c)
{
	...
	if (_lcd.iWriteFlags == 0)
		_lcd.iWriteFlags = DRAW_TO_LCD; // default write mode
	szTemp[0] = c;
	szTemp[1] = 0;
	if (_lcd.pFont == NULL)
	{ // use built-in fonts
...
{
	if (!pDstSprite || iAngle < 0 || iAngle > 359)
		return BB_ERROR_INV_PARAM;
	if (!_lcd.pBackBuffer || !pDstSprite->_lcd.pBackBuffer)
		return BB_ERROR_INV_PARAM;
...
void BB_SPI_LCD::readImage(int x, int y, int w, int h, uint16_t *pixels)
{
	...
	if (!pixels || !_lcd.pBackBuffer || x < 0 || y < 0 || w < 0 || h < 0)
		return;
	if (x + w > _lcd.iCurrentWidth || y + h > _lcd.iCurrentHeight)
		return;
